...
def user_detail_view(request):
    ...
    try:
        if empId=='' and fname == "" and des == "":
            ...
            reportingManagerId = ReportingManagerAssignment.objects.get(user=id).id

            fullNameOfRM = User.objects.get(id=reportingManagerId).first_name+" "+User.objects.get(id=reportingManagerId).last_name
            ...
            userDetails = {
                ...
                "isAdmin": user.isAdmin,
                "casual_leave_days": user.casual_leave_days,
                "salary": str(user.salary) if user.salary is not None else "N/A",
                "currency": user.currency,
                "medical_leave_days": user.medical_leave_days,
                "lop_leave_days": user.lop_leave_days if user.lop_leave_days is not None else "N/A",
                "department": user.department,
                "profilePic": str(user.profilePic) if user.profilePic else "N/A",
                "isReportingManager": user.isReportingManager,
                "ReportingManager":fullNameOfRM,
                "designation": user.designation,
                "can_add_employees": user.can_add_employees,
                "can_remove_employees": user.can_remove_employees,
                "can_update_employees": user.can_update_employees,
            ...
            for name in matching_names:
                ...
                userId = user.id 
                reportingManagerId = ReportingManagerAssignment.objects.get(user=userId).id
                fullNameOfRM = User.objects.get(id=reportingManagerId).first_name+" "+User.objects.get(id=reportingManagerId).last_name 
                userDetails = {
                "email": user.email,
                "first_name": user.first_name,
            ...
            for user in users:
                ...
                userId = user.id 
                if list(ReportingManagerAssignment.objects.filter(user=userId)) != []:
                    reportingManagerId = ReportingManagerAssignment.objects.get(user=userId).id
                    fullNameOfRM = User.objects.get(id=reportingManagerId).first_name+" "+User.objects.get(id=reportingManagerId).last_name 
                else:
                    fullNameOfRM = "Not Assigned/Applicable"
                userDetails = {
                    "email": user.email,
                    "first_name": user.first_name,
                    "last_name": user.last_name,
